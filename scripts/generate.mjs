...
async function scrapeLeague(context, code, url) {
  ...
  try {
    ...
    // Attente d’un tableau ou des ranking-rows
    ...
    const entries = await page.evaluate(() => {
      ...
      // Version SPA: ranking-row
      const rr = Array.from(document.querySelectorAll('[data-testid="ranking-row"]'));
      if (rr.length) {
        for (const row of rr) {
          let team =
            clean(row.querySelector('[data-testid="team-name"]')?.textContent) ||
            clean(row.querySelector(".team-name,.name")?.textContent) ||
            "";
          ...
          const mPts = txt.match(/(?:pts?|points?)\s*:?\s*(-?\d+)/i);
          if (mPts) pts = parseInt(mPts[1], 10);

          const mDiff = txt.match(/(?:diff|±|\+\/-)\s*:?\s*([+\-]?\d+)/i);
          if (mDiff) diff = parseInt(mDiff[1], 10);

          if (team && Number.isFinite(pts)) results.push({ team, pts, diff });
        }
        return results;
      }

      // Fallback: table
      const thead = Array.from(document.querySelectorAll("thead th")).map((th) =>
        clean(th.textContent).toLowerCase()
      );
      const idxTeam = thead.findIndex((h) => /(équipe|equipe|team)/.test(h));
      const idxPts = thead.findIndex((h) => /^(pts|points)\b/.test(h));
      const idxDiff = thead.findIndex((h) => /(diff|±|\+\/-)/.test(h));
      ...
      for (const tr of lines) {
        ...
        let team = idxTeam >= 0 ? texts[idxTeam] : texts[1] || texts[0];
        team = team.split(" — ")[0].split(" - ")[0].split(",")[0];

        let pts = idxPts >= 0 ? onlyInt(texts[idxPts]) : NaN;
        let diff = idxDiff >= 0 ? onlyInt(texts[idxDiff]) : 0;
    ...
    const data = new Map();
    for (const { team, pts, diff } of entries) {
      const name = canonicalName(team);
    ...
    console.warn(`⚠️ ${code}: échec (${e?.message || e}).`);
    return new Map();
  } finally {
    await page.close().catch(() => {});
  }
